# Scoops adaptive learning v63

Date: 2026-08-09

## v63 adaptive setup dropdown clipping fix

- Fixed custom setup dropdowns being clipped by the rounded setup dialog when they open close to the bottom edge.
- The dropdown placement logic now considers both the browser viewport and the visible adaptive-setup shell boundary.
- The extra/specialized reading-support (IEP/504/support) question now opens upward when there is not enough visible room below, keeping every answer choice inside the setup card.
- The change applies to all Scoops custom selects inside the adaptive setup without changing the underlying saved values or validation logic.
...
## v59 iPhone/iPad scroll stability
...
## v59 Create finish and publishing review
...
## v59 layout and library updates
- Stacked the Recommended starting sequence and Why this word path panels to remove the large empty Parent Section gap.
- Prevented the Create Illustration toolbar from overlapping/clipping the Illustration label on wide layouts.
- Added a coded trash-can button to Draft rows in My Library with a confirmation step; only Draft-status stories can be deleted.


## v59 updates
- Spelling Bee now uses one stable action button beneath the answer area: Check Spelling changes to Next Word after a correct answer or after the third attempt.
- New spelling words explicitly clear the prior typed answer and reset all answer styling/state.
- Friends on phone layouts now uses compact horizontal request, best-friend, and friend rows with reduced decoration and visual density.


## v59 Spelling Bee input focus
- The typed spelling field automatically receives focus as each new word loads.
- After an incorrect first or second attempt, focus returns to the answer field so the child can immediately correct the spelling.
- Next Word clears the prior answer and transfers focus directly to the new word input without requiring another tap.


## v59 Friends layout
- Removed the large Friends safety alert.
- Reorganized Friends into three distinct sections: Friend Requests, Best Friend, and Your Friends.
- Replaced dense profile cards with compact rows that include each friend’s latest book.
- Moved report/block access into a small three-dot profile menu.
- Removed the competing Popular panel from the Friends page.


## v59 Friends profiles and search
- Added a working Friends search bar for username/display-name discovery.
- Added real friend profile pages with favorite genres, Books Created, Books Read count only, achievements, latest published book, published books, selected reading favorites, friends preview, and three-dot safety controls.
- Search results distinguish existing friends, requests, and discoverable profiles; new connections route through parent approval.
- Replaced the View Profile “coming soon” behavior with the actual profile view.


## v59 mobile layout repair
- Removed the literal `\n` text accidentally rendered at the bottom of Create.
- Reflowed My Library on phones so Sticker Wall and Next Sticker form a full-width section above the book shelves.
- Mobile Sticker Wall now expands in normal document flow and is constrained to the library width, preventing sticker overflow.


## v60 book reading flow
- Added a clear Read Now button at the bottom-right of the Explore book detail popup, directly beneath the description.
- Read Now opens a dedicated, mobile-responsive reading surface with the book title, author, complete story text, Back, and Done Reading controls.
- Merely opening a book preview no longer counts as reading progress; the reading event now begins when Read Now is selected.
- Done Reading stores a lightweight local completion entry for the prototype so the flow is ready to move to the future cloud database.


## v61 reading, favorites, safety, and notifications

- Continue Reading is now driven by actual local reading progress. Starting a book creates a saved place; leaving the reader keeps it; Done Reading removes it from Continue Reading and records the completed read.
- Book details now show genre and difficulty above a short description. Read Now/Continue Reading remains the primary bottom-right action.
- Readers can favorite/unfavorite books from the detail modal. Favorites appear in My Library and do not expose a full reading history.
- Published books can be reported with a short reason flow; prototype reports are stored locally for later moderation-backend integration. Own books do not show the report control. Friend-book previews also expose the report action.
- The reader includes A− / A+ text-size controls and a comfortable/extra line-spacing control; preferences persist locally.
- Daily Scoop examples no longer use likes, follows, follower counts, or following language. Notifications now focus on friend requests/approvals, story review/publishing, achievements, and learning updates.
- v60's accidental literal `\n` page text was also removed again so Create does not regress.


## v62 child-name consistency

- Fixed the Parent Section bug where the profile card and personalized-learning-path heading could show different child names.
- Added one shared live child-name resolver so the active Scoops profile/session name is used consistently instead of a stale `account.childName` value winning in later adaptive-learning renderers.
- Parent Section hero text, student strip, personalized learning path, parent verification emails, progress-report naming, My Library/profile labels, and current-child book ownership now resolve against the same child name.
- Added a final Parent Section resync after late adaptive renderers run so an older name cannot be reintroduced after the page first displays the correct one.
