...
const SCOOPS_BUILD = 'progress-reports-v6-2026-08-02';
const WELCOME_TEMPLATE_VERSION = 'welcome-branded-v3';
const REPORT_TEMPLATE_VERSION = 'progress-balanced-v2';
...
function reportProgressRow(label, value, fill, attempts = 0) {
  ...
  return `<div style="margin:0 0 15px;">
    <div style="display:flex;justify-content:space-between;gap:10px;margin-bottom:6px;font-size:13px;font-weight:800;color:#444;">
      <span>${escapeHtml(label)}</span><span>${attempts > 0 ? `${percentage}%` : 'No score yet'}</span>
    </div>
    <div style="height:11px;background:#eeeeee;border-radius:999px;overflow:hidden;">
      <div style="height:11px;width:${attempts > 0 ? percentage : 0}%;background:${fill};border-radius:999px;"></div>
    </div>
    <div style="margin-top:5px;font-size:11px;font-weight:800;color:${attempts <= 0 ? '#777' : percentage < 75 ? '#b23b52' : '#4b7650'};">${escapeHtml(status)}${attempts > 0 ? ` · ${attempts} assessment${attempts === 1 ? '' : 's'}` : ''}</div>
  </div>`;
}
