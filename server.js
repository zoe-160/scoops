...
const SCOOPS_BUILD = 'adaptive-learning-v15-2026-08-03';
const WELCOME_TEMPLATE_VERSION = 'welcome-inline-profile-v5';
const REPORT_TEMPLATE_VERSION = 'adaptive-path-reading-detective-v11';
const PDF_TEMPLATE_VERSION = 'reading-detective-progress-pdf-v4';
...
function reportReadingDetectiveHtml(profile) {
  ...
  return `<div style="font-size:12px;line-height:1.65;color:#444;"><strong>Latest passage rate:</strong> ${escapeHtml(rate)}<br><strong>Understanding:</strong> ${escapeHtml(comprehension)}<br><strong>Pattern:</strong> ${escapeHtml(profile.latestPattern || 'More attempts are needed to identify a stable pattern.')}<br><strong>Highlighting:</strong> ${escapeHtml(profile.averageHighlightCount || 0)} important section${Number(profile.averageHighlightCount || 0) === 1 ? '' : 's'} per completed passage on average.</div><div style="margin-top:8px;font-size:10px;line-height:1.45;color:#667;">Reading-rate patterns should be reviewed across several passages and are not diagnostic.</div>`;
}
