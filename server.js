...
const SCOOPS_BUILD = 'progress-reports-v8-2026-08-02';
const WELCOME_TEMPLATE_VERSION = 'welcome-rounded-logo-v4';
const REPORT_TEMPLATE_VERSION = 'progress-comparison-details-v4';
  ...
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;border-collapse:collapse;background:linear-gradient(160deg,#a8d8f0 0%,#c5e8f7 100%);">
    <tr><td align="center" style="padding:0;">
      <div class="scoops-email-stage" style="position:relative;max-width:850px;margin:0 auto;padding:28px 26px 46px;overflow:hidden;">
        <div class="scoops-email-logo" style="position:relative;z-index:3;text-align:center;margin:0 0 18px;font-family:'Fredoka One','Arial Rounded MT Bold','Trebuchet MS',Arial,sans-serif;font-size:43px;font-weight:400;letter-spacing:1.5px;color:#7c5cbf;text-shadow:3px 3px 0 #b39ddb;line-height:1.05;">Scoops</div>
...
function normalizeProgressReport(input) {
  ...
  ['phonics', 'vocabulary', 'spelling'].forEach(key => {
...
function reportMetricCard(value, label, accent, secondary = 'Current period') {
  return `<td width="33.33%" style="padding:4px;vertical-align:top;">
    <div style="padding:12px 8px 9px;text-align:center;background:#ffffff;border:2px solid ${accent};border-radius:15px;">
      <div style="font-size:23px;font-weight:900;color:#5f469d;line-height:1.1;">${escapeHtml(value)}</div>
      <div style="margin-top:5px;font-size:11px;font-weight:800;color:#666;line-height:1.25;">${escapeHtml(label)}</div>
      <div style="margin-top:7px;padding-top:6px;border-top:1px solid #eeeeee;font-size:10px;font-weight:800;color:#6c6c6c;line-height:1.35;">${escapeHtml(secondary)}</div>
    </div>
  </td>`;
}
...
async function sendProgressReportEmail(email, rawReport, automatic = false) {
  ...
  const detailedText = report.mistakeDetails.filter(item => item.attempts || item.missed).flatMap(item => {
    const lines = [`${item.area}: ${item.summary}`];
    if (item.repeatedSkills.length) lines.push(`Repeated pattern: ${item.repeatedSkills.map(skill => `${skill.skill} (${skill.count})`).join(', ')}`);
    if (item.missedItems.length) lines.push(`Frequently missed: ${item.missedItems.map(entry => `${entry.item}${entry.count > 1 ? ` (${entry.count} times)` : ''}`).join(', ')}`);
    if (item.examples[0]) lines.push(`Example: ${item.examples[0].question} — chose "${item.examples[0].selectedAnswer}"; correct answer: "${item.examples[0].correctAnswer}".`);
    return lines;
  });
  ...
  const text = [
    ...
    '',
    `${comparison.overallDirection}: ${comparison.overallMessage}`,
    comparison.available ? `Learning minutes: ${comparisonLine(comparison.metrics.minutes)}` : '',
    comparison.available ? `Lessons completed: ${comparisonLine(comparison.metrics.lessons)}` : '',
    comparison.available ? `Quiz average: ${comparisonLine(comparison.metrics.quizAverage)}` : '',
    comparison.available ? `Stories created: ${comparisonLine(comparison.metrics.stories)}` : '',
    '',
  ...
  const improvementHtml = report.improvementAreas.length
    ? `<ul style="margin:0;padding-left:20px;font-size:14px;line-height:1.7;color:#51452f;">${report.improvementAreas.map(item => `<li style="margin-bottom:9px;"><strong>${escapeHtml(item.area)}: ${item.score}% — ${escapeHtml(item.status)}</strong><br>${escapeHtml(item.message)}</li>`).join('')}</ul>`
    : `<div style="font-size:14px;line-height:1.65;color:#51452f;">No assessed area is currently below the 75% target. Scoops will continue watching for declining results and repeated mistake patterns.</div>`;
  const unassessedHtml = report.unassessedAreas.length
    ? `<div style="margin-top:12px;padding-top:11px;border-top:1px solid #e4d2a6;font-size:12px;line-height:1.55;color:#6c6048;"><strong>Not yet assessed in this period:</strong> ${escapeHtml(report.unassessedAreas.join(', '))}. Missing activity is not treated as a failing score.</div>` : '';
  const mistakeHtml = report.mistakeDetails.map(reportMistakeHtml).filter(Boolean).join('') || `<div style="font-size:14px;line-height:1.65;color:#555;">Question-level results have not been recorded yet. New quizzes will identify specific missed words, selected answers, correct answers, and repeated skill patterns.</div>`;
  ...
  const card = `<div style="overflow:hidden;background:#ffffff;border:3px solid #b39ddb;border-radius:24px;box-shadow:0 9px 26px rgba(84,64,115,.15);">
    <div style="padding:27px 28px 24px;background:linear-gradient(135deg,#f3eeff,#fce4ec);border-bottom:2px solid #e0d4f7;">
      ...
      <div style="display:inline-block;margin-top:11px;padding:7px 12px;background:#ffffff;border:2px solid #b39ddb;border-radius:999px;font-size:12px;font-weight:900;color:#6748ad;">${escapeHtml(comparison.overallDirection)}</div>
    </div>

    <div style="padding:24px 22px 28px;">
      <p style="margin:0 0 16px;font-size:16px;line-height:1.6;">This report shows what is improving, what is moving in the wrong direction, and the exact skills or quiz items that may need more practice.</p>

      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:separate;margin:0 -4px 16px;width:calc(100% + 8px);">
        <tr>
      ...
      <div style="margin:0 0 16px;padding:17px;background:#f8f5fc;border:2px solid #d9cbed;border-radius:17px;">
        <div style="margin-bottom:5px;font-size:16px;font-weight:900;color:#6748ad;">Compared with the preceding period</div>
        <div style="margin-bottom:11px;font-size:13px;line-height:1.6;color:#555;"><strong>${escapeHtml(comparison.overallDirection)}</strong> — ${escapeHtml(comparison.overallMessage)}</div>
        ${comparison.available ? `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:separate;margin:0 -4px;width:calc(100% + 8px);"><tr>${reportComparisonCell('Learning minutes', comparison.metrics.minutes)}${reportComparisonCell('Lessons completed', comparison.metrics.lessons)}</tr><tr>${reportComparisonCell('Quiz average', comparison.metrics.quizAverage)}${reportComparisonCell('Stories created', comparison.metrics.stories)}</tr></table>` : ''}
      </div>

      <div style="margin:0 0 16px;padding:18px;background:#fcfaff;border:2px solid #e0d4f7;border-radius:17px;">
        <div style="margin-bottom:12px;font-size:16px;font-weight:900;color:#6748ad;">Learning-path progress</div>
        ${reportProgressRow('Fun with Phonics', report.categoryDetails.phonics, '#9b7bd2')}
        ${reportProgressRow('Vocabulary Vault', report.categoryDetails.vocabulary, '#ffb74d')}
        ${reportProgressRow('Spelling Bee', report.categoryDetails.spelling, '#81c784')}
        <div style="font-size:11px;line-height:1.5;color:#777;">Assessment counts show how many completed quizzes were used to calculate each average. They do not represent a number of failed assessments.</div>
      </div>

      <div style="margin:0 0 16px;padding:18px;background:#ffffff;border:2px solid #e0d4f7;border-radius:17px;">
        <div style="margin-bottom:7px;font-size:16px;font-weight:900;color:#6748ad;">What the results show</div>
        <div style="font-size:13px;line-height:1.6;color:#555;">Scoops records the skill, word or question, selected answer, correct answer, and whether the same difficulty appears again.</div>
        ${mistakeHtml}
      </div>

      <div style="margin:0 0 16px;padding:18px;background:#fffdf4;border:2px solid #ffe082;border-radius:17px;">
        <div style="margin-bottom:9px;font-size:16px;font-weight:900;color:#765a00;">What went well</div>
        <ul style="margin:0;padding-left:20px;font-size:14px;line-height:1.7;color:#444;">${highlights.map(item => `<li>${escapeHtml(item)}</li>`).join('')}</ul>
      </div>

      <div style="margin:0 0 16px;padding:18px;background:#fff9eb;border:2px solid #f0c477;border-radius:17px;">
        <div style="margin-bottom:9px;font-size:16px;font-weight:900;color:#765213;">Where improvement is needed</div>
