...
const SCOOPS_BUILD = 'progress-reports-v12-2026-08-02';
const WELCOME_TEMPLATE_VERSION = 'welcome-inline-profile-v5';
const REPORT_TEMPLATE_VERSION = 'progress-comparison-strip-v8';
const PDF_TEMPLATE_VERSION = 'progress-pdf-v2';
...
function createProgressReportPdf(rawReport) {
  ...
  const areaColors = { phonics: '#9b7bd2', vocabulary: '#ffb74d', spelling: '#81c784' };
  for (const key of ['phonics', 'vocabulary', 'spelling']) {
...
function reportMistakeCell(detail) {
  if (!detail || (!detail.attempts && !detail.missed)) {
    return `<td class="scoops-three-col" width="33.33%" style="width:33.33%;padding:4px;vertical-align:top;"><div style="height:100%;padding:13px;background:#fcfaff;border:1px solid #e0d4f7;border-radius:13px;font-size:12px;line-height:1.55;color:#555;"><strong style="display:block;margin-bottom:5px;color:#6748ad;">Not assessed</strong>Question-level results have not been recorded for this area yet.</div></td>`;
  }
  const repeated = detail.repeatedSkills?.length
...
async function sendProgressReportEmail(email, rawReport, automatic = false) {
  ...
  const card = `<div style="overflow:hidden;background:#ffffff;border:3px solid #b39ddb;border-radius:24px;box-shadow:0 9px 26px rgba(84,64,115,.15);">
    <div style="padding:23px 25px 20px;background:linear-gradient(135deg,#f3eeff,#fce4ec);border-bottom:2px solid #e0d4f7;">
      ...
      <div style="display:inline-block;margin-top:10px;padding:6px 11px;background:#ffffff;border:2px solid #b39ddb;border-radius:999px;font-size:11px;font-weight:900;color:#6748ad;">${escapeHtml(comparison.overallDirection)}</div>
    </div>
    ...
    <div style="padding:20px 20px 24px;">
      ...
      <div style="margin:0 0 14px;padding:15px;background:#fcfaff;border:2px solid #e0d4f7;border-radius:16px;">
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;border-collapse:separate;margin:0 -6px;width:calc(100% + 12px);">
          <tr>
            <td class="scoops-three-col" width="33.33%" style="width:33.33%;padding:6px;vertical-align:top;">${reportProgressRow('Fun with Phonics', report.categoryDetails.phonics, '#9b7bd2')}</td>
            <td class="scoops-three-col" width="33.33%" style="width:33.33%;padding:6px;vertical-align:top;">${reportProgressRow('Vocabulary Vault', report.categoryDetails.vocabulary, '#ffb74d')}</td>
            <td class="scoops-three-col" width="33.33%" style="width:33.33%;padding:6px;vertical-align:top;">${reportProgressRow('Spelling Bee', report.categoryDetails.spelling, '#81c784')}</td>
          </tr>
        </table>
      ...
      <div style="margin:0 0 14px;padding:15px;background:#ffffff;border:2px solid #e0d4f7;border-radius:16px;">
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;border-collapse:separate;margin:0 -4px;width:calc(100% + 8px);"><tr>
          ${reportMistakeCell(report.categoryDetails.phonics?.findings || report.mistakeDetails.find(item => item.area === 'Phonics'))}
